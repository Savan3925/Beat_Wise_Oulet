...
def get_manday():
    return process_report(get_file_path('CP_Master.csv'))

@app.route('/Beat_Report_Final_2', methods=['GET'])
def get_manday_2():
    return process_report(get_file_path('CP_Master2.csv'))

@app.route('/Beat_Report_By_BeatName', methods=['GET'])
def get_beat_name_report():
    return process_beat_report(get_file_path('Beat_Salesman_Name.csv'))

def process_report(filepath):
    try:
        df = pd.read_csv(filepath, sep='|', quotechar='"', skipinitialspace=True, encoding='utf-8').fillna('')
...
def process_beat_report(filepath):
    try:
        df = pd.read_csv(filepath, sep='|', quotechar='"', skipinitialspace=True, encoding='utf-8').fillna('')
